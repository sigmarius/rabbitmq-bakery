...
## Скриншот теста всех сервисов

![Тестовый запуск основных сервисов](docs/screenshots/test-services.jpg)

На скриншоте видно типичный сценарий «все сервисы работают одновременно»:

- слева/сверху консоль контейнера RabbitMQ (статус `Up ... (healthy)` и проброс портов, чтобы было видно, что брокер жив);
- отдельно запущен `warehouse_service.php` — он ждёт и обрабатывает заказы из очереди сборки `q.orders.picking` (в логе видно строки вида `Сборка заказа ...`);
- отдельно запущен `courier_service.php` — он ждёт доставку и публикует `order.delivered` (в логе видно `Доставляю ...`);
- ниже консоли Docker/контейнера показан «тестовый вход» `make order`: последовательно запускаются клиенты, которые формируют события заказа и отложенное напоминание.
  - сначала запускается `php .\bin\shop_service.php` (AMQP): в консоли видно `[shop] Оплачен заказ ... (AMQP + отложенное напоминание ... ms)` — это значит, что событие заказа ушло в `ex.orders` и дополнительно создано отложенное сообщение через delayed exchange;
  - затем запускается `php .\bin\mqtt_publish_order.php` (MQTT): в консоли видно `[mqtt] Опубликовано ... маршрут amq.topic -> ... -> q.orders.picking` — это значит, что MQTT-сообщение корректно прокинулось в ту же очереди склада `q.orders.picking`;
  - затем снова запускается `php .\bin\shop_service.php` (AMQP) — для очередной проверки, что цепочка событий стабильно работает при повторных заказах.
- отдельно запущен `sms_reminder_service.php` — он читает сообщения из очереди `q.orders.sms_sim` (в логе видно «Напоминание клиенту: ...» после задержки);
- параллельно выполняется производственная цепочка, но её консоли на этом конкретном скрине не показаны:
  - `schedule_bake.php` отправляет задание в `ex.production` (direct) → очередь `q.production.bake`;
  - `production_service.php` забирает задание из `q.production.bake` и публикует отчёт о расходе сырья в `ex.inventory` (fanout) и при необходимости в `ex.alerts` (headers).
  - `procurement_service.php` в отдельном терминале подписан на `q.inventory.signals` и выводит сигналы в консоль (на скрине это не отображено), поэтому видно, что обработка «сырьевой» ветки тоже реально происходит.

Это свидетельствует о том, что связка «магазин → склад → курьер → завершение заказа → обработка сигналов (procurement) → delayed уведомления (sms)» в проекте реально работает и сервисы не ждут друг друга в синхронном режиме (всё расходится по очередям).
